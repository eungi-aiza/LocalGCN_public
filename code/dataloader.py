...
class Loader(Dataset):
    def __init__(self, config=world.config, path=join(world.DATA_PATH, "ml-1m")):
    ...
    def get_sparse_graph_groups(self, group_embedding):
        """
        input : tensor of shape (users+items, classes)
                - only the highest class of each user is 1
                - all values of each item are 1
        """
        group_embedding = group_embedding.transpose(0, 1).cpu()  # (groups, users+items)
        g_group = []
        Graph = self.Graph.cpu()

        # k groups
        for k in tqdm(range(self.group)):
            # k group's norm_matrix
            g_item = torch.mul(
                group_embedding[k]
                .expand(self.n_users + self.m_items, self.n_users + self.m_items)
                .sparse_mask(Graph.coalesce()),
                Graph,
            )
            g_item = torch.mul(
                group_embedding[k]
                .view(-1, 1)
                .expand(self.n_users + self.m_items, self.n_users + self.m_items)
                .sparse_mask(g_item.coalesce()),
                g_item,
            )

            g_item = self._remove_zeros_from_sparse_tensor(g_item)
            g_group.append(g_item)
