...
def BPR_train_original(dataset, model, bpr, epoch, neg_k=1, w=None):
    ...
    aver_loss = 0.0
    train_loader = tqdm(enumerate(utils.minibatch(users, pos_items, neg_items, batch_size=world.config["bpr_batch_size"])))
...
def evaluate(dataset, model, epoch, patience, best_score, save_dict=None, w=None, multicore=0, analysis=None):
