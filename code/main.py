...
def load_model(model_name):
    models = {"localgcn": model.LocalGCN, "anchorrec": model.AnchorEmbRec, "lgn": model.LightGCN}
    return models[model_name]

def load_dataset(dataset_path):
    return dataloader.Loader(path=dataset_path)

def test_only(dataset, model, analysis=None):
    world.LOGGER.info("[TEST]")
    test_info, _, _ = trainer.evaluate(
        dataset,
        model,
        epoch=0,
        patience=0,
...
if __name__ == "__main__":
    ...
    dataset = load_dataset(os.path.join(world.DATA_PATH, world.config["dataset"]))
    ...
    # Anchor Emb Rec model
    if world.config["model"] == "anchorrec":
        test_only(dataset, model, world.config['analysis'])
    ...
    start_epoch = 0
    if world.config["load"] or world.config["analysis"] == 1:
        try:
    ...
    try:
        ...
        for epoch in range(start_epoch, world.config["epochs"]):
            train_info, save_dict = trainer.BPR_train_original(dataset, model, bpr, epoch, neg_k=world.config["neg_k"], w=w)
